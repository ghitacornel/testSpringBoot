...
public class TestElasticSearch {
    ...
    public void testSearchParentChildProjection() throws Exception {
        MvcResult result = mvc.perform(get("/elastic/parent/children/projection/{content}", "some detailed content here"))
                .andExpect(status().isOk())
                .andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON))
                .andReturn();
        System.err.println(result.getResponse().getContentAsString());
    ...
    public void testSearchSimple() throws Exception {
        MvcResult result = mvc.perform(get("/elastic/simple/{content}", "some additional stuff"))
                .andExpect(status().isOk())
                .andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON))
                .andReturn();
        System.err.println(result.getResponse().getContentAsString());
